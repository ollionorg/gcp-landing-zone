...
module "base_transitivity" {
  ...
  gw_subnets          = { for region in keys(local.base_subnet_primary_ranges) : region => "sb-${local.shared_environment_code}-shared-base-hub-${region}" }
...
module "restricted_transitivity" {
  ...
  gw_subnets          = { for region in keys(local.restricted_subnet_primary_ranges) : region => "sb-${local.shared_environment_code}-shared-restricted-hub-${region}" }
