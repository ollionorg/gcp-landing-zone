...
data "google_compute_network" "shared_vpc" {
  name    = "${local.vpc_prefix}-${local.dev_env_code}-shared-base${local.shared_vpc_mode}"
...
module "peering_network" {
  ...
  network_name                           = "${local.vpc_prefix}-${local.dev_env_code}-${var.peer_base}"
...
module "peering" {
  ...
  prefix            = "${var.bu_code}-${local.dev_env_code}"
...
resource "google_compute_firewall" "deny_all_egress" {
  name      = "${var.peer_fw_name}-${local.dev_env_code}-${var.peer_base}-65535-e-${local.dev_env_code}-all-all-tcp-udp"
...
resource "google_compute_firewall" "allow_private_api_egress" {
  name      = "${var.peer_fw_name}-${local.dev_env_code}-${var.peer_base}-65534-e-a-allow-google-apis-all-tcp-443"
  network   = module.peering_network.network_name
  project   = module.peering_project.project_id
  direction = "EGRESS"
  priority  = 65534
...
resource "google_compute_firewall" "allow_iap_ssh" {
  ...
  name    = "${var.peer_fw_name}-${local.dev_env_code}-${var.peer_base}-1000-i-a-all-allow-iap-ssh-tcp-22"
  network = module.peering_network.network_name
  project = module.peering_project.project_id
...
resource "google_compute_firewall" "allow_iap_rdp" {
  count   = var.optional_fw_rules_enabled ? 1 : 0
  name    = "${var.peer_fw_name}-${local.dev_env_code}-${var.peer_base}-1000-i-a-all-allow-iap-rdp-tcp-3389"
  network = module.peering_network.network_name
  project = module.peering_project.project_id
...
resource "google_compute_firewall" "allow_windows_activation" {
  ...
  name      = "${var.peer_fw_name}-${local.dev_env_code}-${var.peer_base}-0-e-a-allow-win-activation-all-tcp-1688"
  network   = module.peering_network.network_name
  project   = module.peering_project.project_id
  direction = "EGRESS"
  priority  = 0
...
resource "google_compute_firewall" "allow_lb" {
  count   = var.optional_fw_rules_enabled ? 1 : 0
  name    = "${var.peer_fw_name}-${local.dev_env_code}-${var.peer_base}-1000-i-a-all-allow-lb-tcp-80-8080-443"
  network = module.peering_network.network_name
  project = module.peering_project.project_id
